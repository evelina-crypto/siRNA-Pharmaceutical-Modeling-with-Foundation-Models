...
#### Three-branch model (sequence + experimental + mRNA)

# this model has a third input, so each row also carries the mRNA embeddings and a
# mask of which regions exist. it gets its own dataset and loops, so it does not
# break the two-tensor runtime version above.

class IndexedMRNADataset(torch.utils.data.Dataset):
    """sequence, experimental, the mRNA region embeddings and their present mask."""

    def __init__(self, X_seq_tensor, X_exp_tensor, X_mrna_tensor, mrna_mask_tensor, y_tensor, indices):
        self.X_seq = X_seq_tensor
        self.X_exp = X_exp_tensor
        self.X_mrna = X_mrna_tensor
        self.mrna_mask = mrna_mask_tensor
        self.y = y_tensor
        self.indices = indices
    ...
    def __getitem__(self, idx):
        return (self.X_seq[idx], self.X_exp[idx], self.X_mrna[idx],
                self.mrna_mask[idx], self.y[idx], self.indices[idx])


def train_model_mrna(model, train_loader, val_loader, criterion, optimizer,
                     epochs=1000, device='cpu', patience=15):
    from modeling.training_utils import EarlyStopping
    ...
    for epoch in range(epochs):
        ...
        for X_seq, X_exp, X_mrna, mrna_mask, y, _ in train_loader:
            X_seq, X_exp = X_seq.to(device), X_exp.to(device)
            X_mrna, mrna_mask, y = X_mrna.to(device), mrna_mask.to(device), y.to(device)
            y = y.squeeze(-1)
            optimizer.zero_grad()
            y_pred = model(X_seq, X_exp, X_mrna, mrna_mask).flatten()
            loss = criterion(y_pred, y.float())
            loss.backward()
            optimizer.step()
            train_loss += loss.item()
        ...
        val_loss = 0.0
        with torch.no_grad():
            for X_seq, X_exp, X_mrna, mrna_mask, y, _ in val_loader:
                X_seq, X_exp = X_seq.to(device), X_exp.to(device)
                X_mrna, mrna_mask, y = X_mrna.to(device), mrna_mask.to(device), y.to(device)
                y = y.squeeze(-1)
                y_pred = model(X_seq, X_exp, X_mrna, mrna_mask).flatten()
...
def evaluate_model_mrna(scaler_y, model, test_loader, device='cpu'):
    model.eval()
    predictions, actuals, sample_names = [], [], []
    test_loss = 0.0
    criterion = nn.MSELoss()
    ...
    with torch.no_grad():
        for X_seq, X_exp, X_mrna, mrna_mask, y, sample_id in test_loader:
            X_seq, X_exp = X_seq.to(device), X_exp.to(device)
            X_mrna, mrna_mask, y = X_mrna.to(device), mrna_mask.to(device), y.to(device)
            y = y.squeeze(-1)
            y_pred = model(X_seq, X_exp, X_mrna, mrna_mask).flatten()
    ...
    if (np.std(predictions) == 0 or np.std(actuals) == 0
        ...
        test_corr = np.corrcoef(predictions, actuals)[0, 1]
        test_spearman = spearmanr(actuals, predictions).correlation

    test_mse = mean_squared_error(actuals, predictions)

    # spearman is the project metric, report it alongside the rest
    metrics = {'test_loss': test_loss, 'test_correlation': test_corr,
               'test_spearman': test_spearman, 'test_mse': test_mse}
    return metrics, predictions, actuals, sample_names